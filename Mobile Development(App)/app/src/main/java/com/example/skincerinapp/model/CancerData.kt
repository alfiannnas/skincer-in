package com.example.skincerinapp.model

import com.example.skincerinapp.R

object CancerData {
    val cancer = listOf(
        Cancer(
            "1",
            "Actinic keratosis",
            "200\n" +
                    "Patient",
            "Dangerous",
            "Actinic keratosis is a condition in which there are rough, scaly plaques on the skin, which develop as a result of chronic sun exposure. It is most commonly found on the face, lips, ears, back of the hands, forearms, head, or neck. The condition, also known as solar keratosis, can progress slowly and generally causes no signs or symptoms other than plaques on the skin. The plaques develop over many years, and first appear commonly after the age of 40. A small percentage of lesions arising from actinic keratosis may develop into skin cancer.",
            "The main cause of actinic keratosis is excessive exposure to sunlight or ultraviolet light. In other words, solar keratosis will be more common in people who are often active in direct sunlight, as well as people who like to use tanning beds or skin blackening tools.\n",
            "Actinic keratosis symptoms generally appear on parts of the skin that are frequently exposed to the sun. However, not everyone who is frequently exposed to the sun will suffer from actinic keratosis.\n" +
                    "If they appear, the changes that can occur in the affected skin areas are:\n" +
                    "1. Rough and thickened skin, which can even become wart-like.\n" +
                    "2. Scaly skin\n" +
                    "3. The skin changes color to reddish or brownish.\n" +
                    "4. The skin feels itchy or hot.\n" +
                    "These skin disorders usually occur on the temples, forehead, scalp, face, lips, ears, neck, arms and back of hands.\n",
            "Actinic keratosis generally resolves on its own. Doctors usually just advise patients to use sunscreen and moisturizers, so that the skin condition does not get worse. However, in certain cases, actinic keratosis needs to be treated with medication, special therapy, and surgery.\n" +
                    "\n" +
                    "1. Medications\n" +
                    "Medications are used when there is a lot of actinic keratosis on the surface of the skin. They are topical medications in the form of creams or gels.\n" +
                    "\n" +
                    "2. Photodynamic therapy (PDT)\n" +
                    "In this procedure, the doctor will apply chemicals to the problematic skin. After that, the doctor will use a special lamp to destroy the actinic keratosis. But, please note that this therapy can cause side effects on the skin, such as redness, swelling, and a burning sensation.\n" +
                    "\n" +
                    "3. Cryotherapy\n" +
                    "In this procedure, the doctor will use liquid nitrogen to freeze and remove the actinic keratosis. Cryotherapy may cause infection, blisters, and changes in the texture and color of the skin on the area affected by solar keratitis.\n" +
                    "\n" +
                    "4. Surgery\n" +
                    "In severe cases, surgery with scraping will be advised by the doctor as a solution to remove the damaged skin cells. Initially, the doctor will inject a local anesthetic to the patient, and then scrape off the damaged cells on the surface of the skin using a curette tool.\n",
            "Protection from UV rays is essential to prevent the development and recurrence of actinic keratosis. If you frequently engage in activities under the sun, the following preventive measures are highly recommended:\n" +
                    "\n" +
                    "1. Use a sunscreen that is waterproof and contains SPF 30 and apply it evenly on parts of the body that are often exposed to the sun.\n" +
                    "2. Limit activities in direct sunlight between 10 am and 3 pm, because the ultraviolet rays emitted are very high.\n" +
                    "3. Protect the skin from sun exposure with long-sleeved clothes, long pants, socks, closed shoes, jackets, and hats.\n" +
                    "4. Avoid using tanning beds. The tanning tool can emit ultraviolet rays and radiation that can damage the skin.\n" +
                    "5. Do regular skin checks if your skin is problematic so that it can be treated immediately if symptoms of solar keratosis are detected.\n",
            "source: https://yankes.kemkes.go.id/view_artikel/1953/keratosis",
            R.drawable.actinic
        ),
        Cancer(
            "2",
            "Basal Cell Carcinoma",
            "200\n" +
                    "Patient",
            "Dangerous",
            "Basal Cell Carcinoma (BCC) is a malignant ne-oplasm of non-keratinized cells in the basal layer of the epidermis, which is locally invasive, aggressive, destructive, and rarely metastasizes. BCC more commonly occurs in the elderly. Exposure to ultraviolet (UV) rays is the main risk factor for BCC, so predilection is areas of the body that are exposed to UV rays, espe-cially the face, ears, and neck. This malignant tumor is characterized by slow tumor growth, rarely causes metastasis, but can cause destruc-tion of surrounding tissue. The etiopathogene-sis associated with BCC is genetic, environ-mental, and most often exposed to ultraviolet light.\n",
            "The primary cause of Basal Cell Carcinoma (BCC) is prolonged and cumulative exposure to ultraviolet (UV) radiation from sunlight or artificial sources, such as tanning beds. UV radiation damages the DNA of skin cells, particularly in the basal layer of the epidermis. This damage can lead to the development of cancerous cells and the formation of BCC tumors.\n",
            "BCC can vary depending on the subtype and location of the tumor. Common symptoms and characteristics of BCC include:\n" +
                    "1. Open sore: BCC often appears as a sore that does not heal, or a persistent open wound. The sore may ooze or crust over and may bleed intermittently.\n" +
                    "2. Reddish or pinkish patch: BCC can present as a reddish or pinkish patch of skin that may be flat or slightly raised. The patch may be shiny or translucent in appearance.\n" +
                    "3. Pearly or waxy bump: BCC can manifest as a pearly or waxy bump on the skin. The bump may be flesh-colored, white, or pink and can have a translucent or shiny surface.\n" +
                    "4. Shiny or pitted skin: BCC may cause the affected skin to appear shiny, smooth, or pitted, resembling a depression or small indentation.\n" +
                    "5. Ulceration or crusting: In some cases, BCC can develop into an ulcerated lesion with a central depression, or it may crust over and form a scab-like covering.\n" +
                    "6. Slow-growing: BCC tends to grow slowly over time. It may initially appear as a small lesion that gradually increases in size over months or years.\n" +
                    "7. Tenderness or pain: In rare instances, BCC may cause tenderness, pain, or itchiness in the affected area, although most BCCs are painless.\n" +
                    "It's important to note that not all BCCs display noticeable symptoms or cause discomfort, especially in the early stages.\n",
            "The primary goal of treatment is to completely remove or destroy the cancer cells while preserving the surrounding healthy tissue. Common treatment modalities for BCC include:\n" +
                    "1. Surgical Excision: This procedure involves cutting out the cancerous growth along with a margin of healthy skin around it. The excised tissue is then sent to a laboratory for further examination to ensure complete removal of the tumor.\n" +
                    "2. Mohs Surgery: Mohs micrographic surgery is a specialized surgical technique used for larger or recurrent BCCs, as well as tumors located in cosmetically sensitive areas. The surgeon removes the tumor layer by layer and immediately examines each layer under a microscope to ensure complete removal while minimizing the removal of healthy tissue.\n" +
                    "3. Curettage and Electrodessication: In this procedure, the cancerous tissue is scraped off using a curette (a spoon-shaped instrument) and then the area is treated with an electric current to destroy any remaining cancer cells. This method is typically suitable for small and superficial BCCs.\n" +
                    "4. Radiation Therapy: Radiation therapy involves the use of high-energy X-rays or other types of radiation to target and destroy cancer cells. It is often considered when surgery is not feasible or when BCCs are located in difficult-to-treat areas.\n" +
                    "5. Topical Medications: Certain topical medications, such as imiquimod or 5-fluorouracil (5-FU), may be prescribed for superficial BCCs. These medications work by stimulating the immune system or inhibiting the growth of cancer cells.\n" +
                    "6. Photodynamic Therapy (PDT): PDT involves applying a light-activated medication to the skin and then exposing it to a specific wavelength of light. This activates the medication, which selectively destroys the cancer cells.\n" +
                    "7. Laser Surgery: Laser therapy uses focused beams of light to vaporize or destroy BCCs. It is commonly used for superficial or early-stage BCCs.\n",
            "Prevention plays a crucial role in reducing the risk of developing Basal Cell Carcinoma (BCC) and other types of skin cancer. Here are some measures that can help prevent BCC:\n" +
                    "1. Sun Protection: Limiting exposure to ultraviolet (UV) radiation is key in preventing BCC. Follow these sun protection practices:\n" +
                    "- Seek shade: Stay in the shade, especially during peak sunlight hours between 10 am and 4 pm when the sun's rays are the strongest.\n" +
                    "- Wear protective clothing: Cover your skin with long-sleeved shirts, pants, and wide-brimmed hats to shield yourself from direct sunlight.\n" +
                    "- Use sunscreen: Apply a broad-spectrum sunscreen with a sun protection factor (SPF) of 30 or higher on all exposed skin. Reapply every two hours or more frequently if swimming or sweating.\n" +
                    "- Avoid Tanning Beds: Avoid using tanning beds and sunlamps, as they emit harmful UV radiation that can increase the risk of BCC and other skin cancers.\n" +
                    "2. Regular Skin Examinations: Perform regular self-examinations to monitor your skin for any changes, including new growths, sores that don't heal, or changes in existing moles. If you notice any suspicious or concerning changes, consult a healthcare professional promptly.\n" +
                    "3. Protect Your Eyes and Lips: Use sunglasses that block both UVA and UVB rays to protect your eyes from UV damage. Additionally, apply lip balm with SPF to protect your lips from sun exposure.\n" +
                    "4. Check for Risk Factors: Be aware of risk factors associated with BCC, such as fair skin, history of frequent sunburns, family history of skin cancer, and certain genetic disorders. If you have these risk factors, take extra precautions and discuss them with your healthcare provider.\n" +
                    "5. Regular Skin Cancer Screenings: Schedule regular skin cancer screenings with a dermatologist, especially if you have a history of skin cancer or other risk factors. These screenings can help detect BCC or other skin cancers at early stages when they are most treatable.\n" +
                    "Remember, prevention is essential, but if you notice any concerning skin changes or suspect skin cancer, consult a healthcare professional for evaluation and appropriate management.\n",
            "source: https://www.indonesianjournalofcancer.or.id/e-journal/index.php/ijoc/article/view/428\n" +
                    "https://www.mayoclinic.org/diseases-conditions/basal-cell-carcinoma/symptoms-causes/syc-20354187\n",
            R.drawable.basal_cell_carcinoma
        ),
        Cancer(
            "3",
            "Benign Keratosis / Seborrheic Keratosis",
            "200\n" +
                    "Patient",
            "Dangerous",
            "A seborrheic keratosis is a common noncancerous (benign) skin growth. People tend to get more of them as they get older.Seborrheic keratoses are usually brown, black or light tan. The growths (lesions) look waxy or scaly and slightly raised. They appear gradually, usually on the face, neck, chest or back.Seborrheic keratoses are harmless and not contagious. They don't need treatment, but you may decide to have them removed if they become irritated by clothing or you don't like how they look.\n",
            "Experts don't completely understand what causes a seborrheic keratosis. This type of skin growth does tend to run in families, so there is likely an inherited tendency. If you've had one seborrheic keratosis, you're at risk of developing others.\n" +
                    "\n" +
                    "A seborrheic keratosis isn't contagious or cancerous.\n",
            "A seborrheic keratosis grows gradually. Signs and symptoms might include:\n" +
                    "\n" +
                    "1. A round or oval-shaped waxy or rough bump, typically on the face, chest, a shoulder or the back\n" +
                    "2. A flat growth or a slightly raised bump with a scaly surface, with a characteristic \"pasted on\" look\n" +
                    "3. Varied size, from very small to more than 1 inch (2.5 centimeters) across\n" +
                    "4. Varied number, ranging from a single growth to multiple growths\n" +
                    "5. Very small growths clustered around the eyes or elsewhere on the face, sometimes called flesh moles or dermatosis papulosa nigra, common on Black or brown skin\n" +
                    "6. Varied in color, ranging from light tan to brown or black\n" +
                    "7. Itchiness\n",
            "Seborrheic keratosis removal can be achieved with one or a combination of the following methods:\n" +
                    "\n" +
                    "1. Freezing the growth. Freezing a growth with liquid nitrogen (cryotherapy) can be an effective way to remove a seborrheic keratosis. It doesn't always work on raised, thicker growths. This method carries the risk of permanent loss of pigment, especially on Black or brown skin.\n" +
                    "2. Scraping (curettage) or shaving the skin's surface. First your doctor will numb the area and then use a scalpel blade to remove the growth. Sometimes shaving or scraping is used along with cryosurgery to treat thinner or flat growths.\n" +
                    "3. Burning with an electric current (electrocautery). First your doctor will numb the area and then destroy the growth with electrocautery. This method can be used alone or with scraping, especially when removing thicker growths.\n",
            "Seborrheic keratosis is a benign skin condition that cannot be completely prevented since its exact cause is unknown and it is often associated with genetic factors and aging. However, there are some general measures you can take to promote overall skin health and potentially reduce the risk or manage seborrheic keratosis:\n" +
                    "\n" +
                    "1. Protect your skin from excessive sun exposure: Ultraviolet (UV) radiation from the \n" +
                    "2. Practice good skincare routine\n" +
                    "3. Avoid irritants and trauma to the skin\n" +
                    "4. Maintain a healthy lifestyle\n",
            "Source: https://www.mayoclinic.org/diseases-conditions/seborrheic-keratosis/symptoms-causes/syc-20353878#:~:text=A%20seborrheic%20keratosis%20(seb%2Do,or%20scaly%20and%20slightly%20raised.",
            R.drawable.benign_keratosis
        ),
        Cancer(
            "4",
            "Dermatofibroma",
            "200\n" +
                    "Patient",
            "Dangerous",
            "Dermatofibromas are harmless growths within the skin that usually have a small diameter. They can vary in color but are typically pink to light brown in light skin and dark brown or black in dark skin. They may appear more pink or darker if a person accidentally irritates them — for example, when shaving.\n" +
                    "\n" +
                    "As they are dense and firm to the touch, many people say that they feel like a small stone underneath or raised above the skin. Most dermatofibromas are painless, but some people experience itching, irritation, or tenderness at the site of the growth.\n",
            "Some researchersTrusted Source theorize that a possible cause is an adverse reaction to a local trauma, such as a small injury or bug bite in the area where the lesion later forms.\n" +
                    "\n" +
                    "Age may be another risk factor, as the growth appears mostly in adults. People with a suppressed immune system may also be more likely to experience dermatofibromas and to have more than one growth.\n" +
                    "\n" +
                    "Multiple dermatofibromas are also more common in people with underlying conditions, especially in those with systemic lupus erythematosus. In some cases, there may not be an obvious cause.\n",
            "Key markers of a dermatofibroma are:\n" +
                    "\n" +
                    "Appearance: A dermatofibroma presents as a round bump that is mostly under the skin.\n" +
                    "Size: The normal range is about 0.5–1.5 centimeters (cm), with most lesions being 0.7–1.0 cm in diameter. The size will usually remain stable.\n" +
                    "Color: The growths vary in color among individuals but will generally be pink, red, gray, brown, or black.\n" +
                    "Location: Dermatofibromas are most common on the legs, but they sometimes appear on the arms, trunk, and, less commonly, elsewhere on the body.\n" +
                    "Additional symptoms: Although they are usually harmless and painless, these growths may occasionally be itchy, tender, painful, or inflamed\n",
            "1. Cortisone Injections can reduce the size of a dermatofibroma, but primarily work to ease associated pain or itchiness.\n" +
                    "2. Cryotherapy, or cryosurgery, removes the dermatofibroma by freezing the cells with nitrogen.\n" +
                    "3. Laser Therapy uses heat to target the overgrowth of cells that lead to dermatofibroma.\n" +
                    "4. Surgical Excision is the most effective method of removal, but can potentially leave behind a scar that’s more visible than the original dermatofibroma.\n",
            "Because the cause of dermatofibromas is still unknown, there’s no sure method for preventing them. Proper wound care may help reduce the risk of a dermatofibroma forming, as well as having a generally healthy immune system.\n",
            "source: - https://www.medicalnewstoday.com/articles/318870#causes-and-risk-factors\n" +
                    "- https://www.westlakedermatology.com/blog/dermatofibroma-treatment-options/ ",
            R.drawable.dermatofibroma
        ),
        Cancer(
            "5",
            "Melanocytic Nevus",
            "200\n" +
                    "Patient",
            "Dangerous",
            "Moles present from birth, or that develop within the first few months after birth, are called congenital melanocytic nevi (CMN). They are fairly common and, in most cases, they do not cause health problems. However, they can carry risks. Most cases of CMN never require treatment, but there are some cases in which the moles may be removed. This is usually done to lower the risk of skin cancer or to improve appearance.\n",
            "Congenital melanocytic nevi are caused by a change in color (pigment) cells of the skin. The moles happen by chance. CMN is not passed down from the parents. There is no way to prevent your child from being born with moles.\n" +
                    "\n" +
                    "In very rare cases, CMN can indicate a condition called neurocutaneous melanosis. Neurocutaneous melanosis can occur when the CMN are large and found on the back. In this case, the moles can affect the spine and brain, and may cause seizures. This type of CMN is also usually just due to chance and is not passed down from the parents.\n",
            "Symptoms of CMN include the presence of moles on the skin from birth or that appear in the first few months of life. These birthmarks range in color from light brown to black. They may darken, get thicker or grow hair as your child ages. They are usually painless.",
            "Signs that CMN may need treatment include :\n" +
                    "1. Pain, itchiness or burning of the mole\n" +
                    "2. Red or irritated skin around the mole\n" +
                    "3. Blood or pus coming from the mole\n" +
                    "4. A change in color to black, blue or red\n" +
                    "5. Rapid growth in size of the mole\n" +
                    "6. Irregular or uneven borders around the mole\n" +
                    "7. A significant change in texture of giant CMN\n" +
                    "\n" +
                    "1. So you can do this :\n" +
                    "2. Cryotherapy\n" +
                    "3. Surgical Removal\n",
            "Use broad-spectrum (UVA/UVB), water-resistant sunblock during outside time. This means SPF 30 or higher whenever your child is outdoors between 10 a.m. and 4 p.m.\n" +
                    "Reapply sunblock every 2 hours and after swimming or sweating.\n" +
                    "Wear protective clothing, sunglasses and hats to avoid sun exposure and sun burns. When possible, stay in the shade.\n" +
                    "Do not let your child use tanning beds.\n" +
                    "Call the doctor if you notice any changes in your child’s moles.\n",
            "Source: https://www.nationwidechildrens.org/conditions/congenital-melanocytic-nevi#:~:text=Congenital%20melanocytic%20nevi%20are%20caused,from%20being%20born%20with%20moles.",
            R.drawable.melanocytic_nevi
        ),
        Cancer(
            "6",
            "Melanoma",
            "200\n" +
                    "Patient",
            "Dangerous",
            "Melanoma, the most serious type of skin cancer, develops in the cells (melanocytes) that produce melanin — the pigment that gives your skin its color. Melanoma can also form in your eyes and, rarely, inside your body, such as in your nose or throat.\n" +
                    "\n" +
                    "The exact cause of all melanomas isn't clear, but exposure to ultraviolet (UV) radiation from sunlight or tanning lamps and beds increases your risk of developing melanoma. Limiting your exposure to UV radiation can help reduce your risk of melanoma.\n",
            "Melanoma occurs when something goes wrong in the melanin-producing cells (melanocytes) that give color to your skin.\n" +
                    "\n" +
                    "Normally, skin cells develop in a controlled and orderly way — healthy new cells push older cells toward your skin's surface, where they die and eventually fall off. But when some cells develop DNA damage, new cells may begin to grow out of control and can eventually form a mass of cancerous cells.\n" +
                    "\n" +
                    "Just what damages DNA in skin cells and how this leads to melanoma isn't clear. It's likely that a combination of factors, including environmental and genetic factors, causes melanoma. Still, doctors believe exposure to ultraviolet (UV) radiation from the sun and from tanning lamps and beds is the leading cause of melanoma.\n" +
                    "\n" +
                    "UV light doesn't cause all melanomas, especially those that occur in places on your body that don't receive exposure to sunlight. This indicates that other factors may contribute to your risk of melanoma.\n",
            "The first melanoma signs and symptoms often are:\n" +
                    "1. A change in an existing mole\n" +
                    "2. The development of a new pigmented or unusual-looking growth on your skin\n" +
                    "Melanoma doesn't always begin as a mole. It can also occur on otherwise normal-appearing skin.\n",
            "1. Immunotherapy: Immunotherapy drugs help stimulate the body's immune system to recognize and destroy cancer cells.\n" +
                    "2. Targeted therapy: Targeted therapy drugs are designed to specifically target genetic mutations or changes in melanoma cells.\n" +
                    "3. Chemotherapy: While traditional chemotherapy is not as effective for melanoma, it may still be used in certain cases where the cancer has spread extensively or other treatment options are not suitable.\n" +
                    "4. Surgical removal of distant metastases: In advanced cases where the melanoma has spread to other organs, surgery may be performed to remove metastatic lesions.\n" +
                    "5. Sentinel lymph node biopsy: In this procedure, the sentinel lymph node (the first node to which cancer is likely to spread) is removed and examined to determine if the cancer has spread to the lymph nodes.\n",
            "You can reduce your risk of melanoma and other types of skin cancer if you:\n" +
                    "1. Avoid the sun during the middle of the day.\n" +
                    "2. Wear sunscreen year-round.\n" +
                    "3. Wear protective clothing.\n" +
                    "4. Avoid tanning lamps and beds.\n" +
                    "5. Become familiar with your skin so that you'll notice changes.\n",
            "Source: https://www.mayoclinic.org/diseases-conditions/melanoma/symptoms-causes/syc-20374884#:~:text=Melanoma%2C%20the%20most%20serious%20type,in%20your%20nose%20or%20throat",
            R.drawable.melanoma
        ),
        Cancer(
            "7",
            "Squamous Cell Carcinoma(SCC)",
            "200\n" +
                    "Patient",
            "Dangerous",
            "Squamous cell carcinoma of the skin is a common form of skin cancer that develops in the squamous cells that make up the middle and outer layers of the skin.\n" +
                    "Squamous cell carcinoma of the skin is usually not life-threatening, though it can be aggressive. Untreated, squamous cell carcinoma of the skin can grow large or spread to other parts of your body, causing serious complications.\n",
            "1. Have a light skin tone and spent lots of time in the sun\n" +
                    "2. Use(d) tanning beds\n" +
                    "3. Take medication to prevent the body from rejecting a transplanted organ\n",
            "Signs and symptoms of squamous cell carcinoma of the skin include:\n" +
                    "1. A firm, red nodule\n" +
                    "2. A flat sore with a scaly crust\n" +
                    "3. A new sore or raised area on an old scar or ulcer\n" +
                    "4. A rough, scaly patch on your lip that may evolve to an open sore\n" +
                    "5. A red sore or rough patch inside your mouth\n" +
                    "6. A red, raised patch or wartlike sore on or in the anus or on your genitals\n",
            "Most squamous cell carcinomas of the skin can be completely removed with relatively minor surgery or occasionally with a medicine applied to the skin.\n" +
                    "\n" +
                    "1. Treatments for very small skin cancers\n" +
                    "- Curettage and electrodessication (C and E).\n" +
                    "- Laser therapy\n" +
                    "- Freezing\n" +
                    "- Photodynamic therapy\n" +
                    "\n" +
                    "2. Treatments for larger skin cancers\n" +
                    "- Simple excision\n" +
                    "- Mohs surgery.\n" +
                    "- Radiation therapy\n" +
                    "\n" +
                    "3. Treatments for skin cancer that spreads beyond the skin\n" +
                    "- Chemotherapy\n" +
                    "- Targeted drug therapy\n" +
                    "-Immunotherapy\n",
            "Avoid the sun during the middle of the day\n" +
                    "1. Wear sunscreen year-round\n" +
                    "2. Wear protective clothing\n" +
                    "3. Avoid tanning beds\n" +
                    "4. Check your skin regularly and report changes to your doctor\n",
            "source: https://www.mayoclinic.org/diseases-conditions/squamous-cell-carcinoma/diagnosis-treatment/drc-20352486",
            R.drawable.squamous_cell_carcinoma
        ),
        Cancer(
            "8",
            "Vascular Lesion",
            "200\n" +
                    "Patient",
            "Dangerous",
            "A type of tumor that forms from cells that make blood vessels or lymph vessels. Vascular tumors may be benign (not cancer) or malignant (cancer) and can occur anywhere in the body. They may form on the skin, in the tissues below the skin, and/or in an organ. There are many types of vascular tumors. The most common type of vascular tumor is hemangioma, which is a benign tumor that usually occurs in infants and goes away on its own.",
            "Vascular lesions can be caused by various factors, and the specific cause often depends on the type of lesion. But mostly,Genetic factors and certain hormones may also play a role.\n",
            "Here are some common symptoms associated with different types of vascular lesions:\n" +
                    "\n" +
                    "1. Hemangiomas:\n" +
                    "Red, pink, or purplish raised bumps or patches on the skin.\n" +
                    "Rapid growth during infancy followed by a slow fading over time.\n" +
                    "Potential complications may include ulceration, bleeding, or infection in severe cases.\n" +
                    "\n" +
                    "2. Malformations:\n" +
                    "Varied symptoms depending on the organ or area affected.\n" +
                    "In the skin, they may appear as birthmarks or discolored patches.\n" +
                    "Pain, swelling, or functional impairments (such as difficulty breathing, swallowing, or speaking) may occur in deeper or internal malformations.\n" +
                    "\n" +
                    "3. Varicose Veins:\n" +
                    "Enlarged, swollen veins that are visible beneath the skin, often in the legs.\n" +
                    "Aching, heaviness, or discomfort in the affected area.\n" +
                    "Itching, skin discoloration, or the development of skin ulcers in severe cases.\n" +
                    "\n" +
                    "4. Aneurysms:\n" +
                    "Most aneurysms do not cause symptoms unless they rupture.\n" +
                    "Symptoms of a ruptured aneurysm may include sudden and severe pain, dizziness, loss of consciousness, rapid heartbeat, low blood pressure, and signs of internal bleeding.\n" +
                    "\n" +
                    "5. Trauma or Injury:\n" +
                    "Symptoms can vary depending on the extent and location of the injury.\n" +
                    "Bruising, swelling, pain, or tenderness in the affected area.\n" +
                    "Bleeding, either visible or internal, depending on the severity of the trauma.\n",
            "1. Medications: \n" +
                    "Certain medications may be used to manage specific symptoms associated with vascular lesions. For example, beta-blockers or corticosteroids may be prescribed to help shrink or reduce the growth of hemangiomas. Medications to manage pain or inflammation may be prescribed for symptomatic relief.\n" +
                    "\n" +
                    "2. Sclerotherapy: \n" +
                    "This is a common treatment for various vascular lesions, including spider veins, varicose veins, and some types of malformations. It involves injecting a solution directly into the affected blood vessels, causing them to collapse and eventually fade away.\n" +
                    "\n" +
                    "3. Laser Therapy: \n" +
                    "Laser treatments, such as pulsed dye laser or intense pulsed light (IPL), are often used for vascular lesions that primarily affect the skin, such as port-wine stains or small hemangiomas. The laser targets the blood vessels, causing them to shrink and fade.\n" +
                    "\n" +
                    "4. Surgical Excision: \n" +
                    "Surgical removal may be considered for certain types of vascular lesions, especially if they are large, causing significant symptoms or functional impairment, or if there is a risk of complications. This approach may involve cutting out the lesion or, in some cases, removing the affected blood vessels or tissue.\n" +
                    "\n" +
                    "5. Embolization: \n" +
                    "This procedure is commonly used for the treatment of arteriovenous malformations (AVMs) or other vascular lesions where abnormal connections exist between arteries and veins. It involves the injection of a substance (such as small particles or a glue-like material) into the blood vessels to block or reduce blood flow to the lesion.\n" +
                    "\n" +
                    "6. Compression Therapy: \n" +
                    "Compression stockings or bandages may be recommended for managing symptoms and preventing complications associated with varicose veins or other conditions involving poor venous circulation.\n",
            "1. Healthy Lifestyle:\n" +
                    "Maintain a balanced diet rich in fruits, vegetables, whole grains, and lean proteins.\n" +
                    "Engage in regular physical activity to promote cardiovascular health.\n" +
                    "Avoid or quit smoking as it can contribute to the development of vascular conditions.\n" +
                    "\n" +
                    "2. Sun Protection:\n" +
                    "Protect your skin from excessive sun exposure by using sunscreen with a high SPF, wearing protective clothing, and seeking shade during peak sun hours. This can help prevent certain types of skin-related vascular lesions.\n" +
                    "\n" +
                    "3. Regular Check-ups:\n" +
                    "Schedule regular check-ups with your healthcare provider to monitor your overall health and detect any early signs or symptoms of vascular conditions.\n" +
                    "If you have a family history of vascular lesions or conditions, inform your healthcare provider so appropriate screenings or preventive measures can be considered.\n",
            "Source: https://lakejacksonent.com/what-are-vascular-lesions-and-how-can-they-be-treated/",
            R.drawable.vascular_lessions
        )
    )
}